...
class TestDatasetComposition:
    ...
    def test_모든_대상이_시장을_갖는다(self) -> None:
        ...
        시장을 종목명으로 찾는 사전에서 `.get(label, "")` 로 읽으면 그 사전에 없는 대상이
        **조용히 빈칸**이 된다. 반면 바로 옆에서 같은 라벨로 대상 속성을 읽는 자리는
        `KeyError` 로 죽는다 — **한 줄 차이로 가드 강도가 정반대**가 된다.
        시장을 **대상 자신의 속성**으로 두면 조회 실패라는 상태 자체가 없다.
