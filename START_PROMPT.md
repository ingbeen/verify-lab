...
- **`src/verify_lab/data/krx_credentials.py`** — `.env` → 환경 변수. `import pykrx` 자체가 로그인을
  시도하므로 호출 측은 이 로더를 부른 뒤에 pykrx 를 import 한다
- **`scripts/data/check_pykrx_etf.py` 와 그 실측 결과** — KODEX 200 의 성질이 전부 확인됐다.
  결과는 `docs/spec/index_extreme_events.md` §8 "pykrx KODEX 200"
- 테스트와 품질 게이트 — `validate_project.py`가 `failed=0 skipped=0`으로 통과하는 상태

**안 되어 있는 것**

- **KODEX 200 데이터** — 수집기가 없다. 착수 전에 아래 "분배락 문제"를 먼저 결정해야 한다
- `measure/`·`report/`·`studies/` 폴더가 비어 있음

## 결정이 필요한 것 — KODEX 200 분배락

실측으로 확인된 사실이며, 수집기 설계가 여기서 갈린다. 근거는 스펙 §8 결론 1·2에 있다.

| 소스 | 기간 | 분배락 |
| --- | --- | --- |
| `get_etf_ohlcv_by_date` | 상장일부터 전 기간(5,881행) 한 번에 | **조정 안 됨** — 매년 4월 말 약 -1.5%p |
| `get_market_ohlcv(adjusted=True)` | **한 번에 3,000행까지** (2014-05-21부터) | 조정됨 (연 약 1.7%) |

스펙 §2는 "수익률 계산은 수정주가 기준"을 확정했으므로 그대로 두면 후자가 필요한데, 전 기간을 덮으려면
**구간을 나눠 이어붙여야 하고 그 정합성은 아직 실측되지 않았다.**
...
| 후보 | 선행 조건 | 비고 |
| --- | --- | --- |
| **KODEX 200 수집기** | 위 분배락 방침 결정 | 첫 단계로 "나눠 받은 수정주가가 이어붙는가"를 KRX 1회 호출로 실측할 것 |
...
**pykrx 가격 컬럼은 `uint32` 라 차분에서 언더플로우가 난다** — 로딩 시 `float64` 변환이 필요하다.
