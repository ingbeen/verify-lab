...
- [ ] 새 폴더는 **커밋하지 않는다.** `/clean-results` 로 정리한다
      → **커밋하지 않았다**(전부 untracked). 정리는 **사용자가 `/clean-results` 를 직접 호출**해야 한다 —
      그 스킬은 모델 호출이 막혀 있고 **절차를 다른 수단으로 대신하지 않는다.** 대상은 8개다:
...
- 2026-09-14 17:40: 품질 검증 통과(1126/0/0). **남은 것은 산출물 폴더 8개 정리 하나이며
  `/clean-results` 는 사용자 직접 호출 전용이다** — 그래서 상태를 Done 으로 올리지 않았다
