...
```bash
# 전 조합 실행 (기본값) — 종목 5개(원본가) × 청산 요일 × 만기월 1~12
...
- 🔴 **「후보가 가장 많아지는 손절선」을 고르지 마십시오.** 기본 실행(**168칸**) 실측으로
  −1.0% 11칸 · −5.0% 38칸 · −7.0% 40칸입니다. 선정 기준은
...
- **하나의 만기월을 고르지 않습니다.** 12달을 전부 산출해 나란히 보고하고,
  **국내 둘은 금요일·목요일 청산 두 벌**을 냅니다 — 만기가 목요일이라 같은 「다음주 금요일」이
...
- 대상은 **전 칸**(대상 × 청산 요일 × 만기월 12 × 방향 2 = **168칸**)이며 목록을 만드는 것은
  `studies/option_expiry/constants.py` 의 `all_cells()` 입니다 — 성적표가 **16,800행**입니다
