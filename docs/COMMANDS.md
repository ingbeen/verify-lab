...
- **가장 먼저 볼 것은 `성적표.csv` 의 `1차 판정` 컬럼**입니다 — **2026-09-16 에 판정표를
  없애고 판정을 성적표로 합쳤습니다.** 맨몸 판정을 보려면 `손절선(%) = 무손절` · `시기 = 전체`
  로 거르고, 손절선을 바꾸면 같은 칸이 **손절을 걸었을 때의 판정**이 됩니다
...
| 파일 | 무엇 | 행 수 |
| --- | --- | ---: |
| `성적표.csv` | 칸 × 손절선 × 시기의 체결 성적과 `1차 판정` | 300 |
| `거래내역.csv` | 체결 원자료 — 차트로 대조하는 자리 | 1,780 |
| `측정.csv` | **해석 재료** — 중앙값 · 기준선 · 우연확률 · 평균-비율 어긋남 · 배당락 | 3 |

**행 수는 확정 3칸 · 손절선 20종 · 시기 5행 기준입니다.** `summary.json` 이 함께 남습니다.
...
- **손절선 값은 인자가 아닙니다.** 격자(무손절 + −1.0%~−10.0%, 0.5%p 간격)를 전부 내며
  확정 손절선 한 행은 `손절선(%)` **한 컬럼 필터**로 골라냅니다 —
  **전부 내는 것은 고르는 것이 아닙니다.** 확정값의 SoT 는
