...
- **전 칸(대상 × 만기월 12 × 방향 2)을 돕니다.** 목록을 만드는 것은
  `studies/option_expiry/constants.py` 의 `all_cells()` 이며, 여기에 칸 수를 적지 않습니다 —
  대상이 늘면 낡습니다
...
```bash
# 전 조합 실행 (기본값) — 종목 5개(원본가) × 만기월 1~12 × 방향 2
# 「만기일 매수 → 다음주 금요일 매도」 매매를 재고 만기월별로 후보 판정한다
poetry run python scripts/run_option_expiry.py

# 종목 하나만 (qqq · spy · dia · kodex200 · kosdaq150)
poetry run python scripts/run_option_expiry.py --dataset kosdaq150
...
- 🔴 **「후보가 가장 많아지는 손절선」을 고르지 마십시오.** 기본 실행(**120칸**) 실측으로
  −1.0% 4칸 · −5.0% 7칸 · −7.0% 6칸입니다(2026-09-17 게이트 기준). 선정 기준은
...
- 선행 조건: `storage/market/` 에 **원본가 5개 파일**(QQQ·SPY·DIA·069500·229200)이 있어야 합니다
- **하나의 만기월을 고르지 않습니다.** 12달을 전부 산출해 나란히 보고하고,
  ~~국내 둘은 금요일·목요일 청산 두 벌을 냅니다~~ **청산 요일 축은 2026-09-16 에 없앴습니다** — 만기가 목요일이라 같은 「다음주 금요일」이
  미국 5거래일 · 한국 6거래일입니다. **2026-09-16 부터 측정과 체결이 그 축을 함께 냅니다**
- 산출물은 `storage/results/매매/옵션_만기일/` 에 8개 측정 CSV 와 체결 산출물, `summary.json` 으로 남습니다.
...
- 대상은 **전 칸**(대상 5 × 만기월 12 × 방향 2 = **120칸**)이며 목록을 만드는 것은
  `studies/option_expiry/constants.py` 의 `all_cells()` 입니다 — 성적표가 **12,000행**입니다
- **청산은 다음 주 «금요일» 하나입니다.** 목요일 청산을 대조로 함께 내던 축을 없앴으므로
  산출물에 `청산 요일` 컬럼이 없습니다 — 근거는 `매매/옵션_만기일/설계.md` 결정 ⑳ 입니다
- **코드가 칸을 고르지 않습니다.** 게이트를 넘은 칸만 적어 두면 시세를 다시 받아 결과가
  바뀌어도 목록이 따라오지 않고, 「왜 이 칸인가」의 근거가 코드에서 사라져 사후 선택과
  구별되지 않습니다. **실제로 거는 칸은 `매매/옵션_만기일/규칙.md` §1 이 정합니다**
- **미국 9월 세 칸(QQQ·SPY·DIA)은 같은 날 같은 방향**이라 독립된 세 번의 기회가 아닙니다
- 산출물은 `storage/results/매매/옵션_만기일/` 에 남습니다
  - `성적표.csv`(대상 칸 × 손절선 × 시기) · `거래내역.csv`(체결 원자료) · `summary.json`
  - 그 옆에 **측정 표 8개**가 함께 있습니다 — 기준선과 우연확률은 `통계.csv` 가 담습니다
...
- **성적표의 무손절·전체 행이 `통계.csv` 의 같은 칸과 맞는지** 확인하세요.
