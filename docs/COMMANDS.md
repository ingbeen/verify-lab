...
- **거래비용·이자·세금과 하단 이탈 A/B 가 모두 반영됐고 사양서 §13 의 지표와 벤치마크 3종이
  함께 나옵니다.** 축별 검사와 국면 분할은 아래 **견고성 검사**가 한 번에 냅니다
- **실행할 때마다 성과 지표와 §15.3 Red Flag 판정이 표로 나옵니다.** 표준 지표 6종(총수익률·
  CAGR·MDD·Calmar·Sharpe·Sortino), 그리드 전용 지표 13종, Red Flag 9개이며 `summary.json` 에도
  같은 값이 실립니다. **판정할 수 없는 항목은 통과가 아니라 「판정 불가」로 적습니다**
- **벤치마크 3종이 매 실행에 병기되고 `benchmarks.csv` 로 저장됩니다** (사양서 §13.3 「필수 병기」).
  B&H(전액 보유) · 분할매수 후 보유 · 원화 파킹 100% 이며 셋 다 **전략과 같은 거래일·같은 가격·
  같은 금리**를 겪습니다. CSV 에는 **거래일별로 네 곡선이 나란히** 실려 어느 국면에서 갈라지는지
  차트로 대조할 수 있습니다
- ⚠️ **전략은 「분할매수 후 보유」에 네 실행 전부 집니다** (−309만 ~ −4,578만원).
  §13.3 은 "여기서 못 이기면 **익절 로직을 제거하는 것이 맞다**" 고 규정했습니다.
  **다만 노출이 4배 다릅니다** — 그쪽 투입률이 83~90% 이고 MDD 가 −14 ~ −34% 입니다.
  **Calmar 는 전략이 4/4, Sharpe·Sortino 는 3/4 로 이깁니다.** 어느 하나로 결론을 정하지 마세요
  ([spec/usdkrw_grid.md](spec/usdkrw_grid.md) §3.14)
- **「원화 파킹 100%」는 무위험 수익률(rf)과 다른 값입니다.** rf 는 CD91 **원지표** 세후라 하한이
  없고, 이 벤치마크는 실수령 파킹 금리 `max(CD91 − 0.30%p, 0.50%)` 입니다 — 21년 기준
  **8,765,920원(8.77%p) 낮으며** 그래서 이 벤치마크의 **Sharpe 가 −2.109 로 음수**입니다.
  계산 오류가 아니라 "무위험 금리보다 낮게 굴렸다"는 정확한 측정입니다
- **B&H 의 첫날 매수에는 슬리피지가 붙지 않습니다.** 슬리피지는 15:20 **돌파 판정**과 종가의
  차이를 흡수한 값인데 B&H 에는 돌파 판정이 없습니다 — 월말 이자 환전과 같은 논리입니다
- **Sharpe 는 0.122 입니다** (환전 2005~ 기본 설정). **rf=0 이면 0.885 로 부풀려지며** 사양서
  §13.1 이 그것을 금지했습니다. rf 는 **CD91 원지표에 15.4% 를 적용한 세후 값**이고
  **달력일 간격만큼** 붙습니다 — 거래일마다 하루치만 주면 주말이 빠져 Sharpe 가 3배가 됩니다.
  §13.1 이 "rf를 올바르게 적용하면 Sharpe가 0 근처로 떨어질 수 있으며 오류가 아니다" 라고
  미리 적었고 그대로 실현됐습니다 ([spec/usdkrw_grid.md](spec/usdkrw_grid.md) §3.13)
- **세후 CD91 에 그냥 넣어뒀다면 21년 뒤 166,319,585원**입니다. 전략은 178,494,280원이니
  **초과분이 12.17%p** 입니다. 「그래서 매매할 가치가 있나」는 이제 벤치마크 3종이 정식으로 답합니다
- **§15.3 에서 걸리는 것은 MDD 하나뿐입니다** (네 경로 전부 −3.19 ~ −8.10%). 원인은 미실현
  손실 누락이 아니라 **투입률**(19~36%)이며, 「자산곡선 단조 증가」가 통과하는 것이 미실현이
  들어 있다는 증거입니다
- **이탈 보너스 비중을 총수익 기준으로만 읽지 마세요.** 네 경로 모두 30% 를 통과하지만
  **매매 기여분 기준으로는 32.53~75.95%** 입니다 — 이자가 분모에 들어가 위험을 가립니다
...
- 산출물은 `storage/results/<실행시각>_usdkrw_grid/` 에 `daily.csv`(일별 총자산 곡선),
  `trades.csv`(체결 내역), `benchmarks.csv`(전략과 벤치마크 3종의 곡선), `summary.json` 으로 남습니다
- `benchmarks.csv` 는 **거래일 한 줄에 네 곡선**(전략·B&H·분할매수 후 보유·원화 파킹 100%)입니다.
  §13.3 의 질문이 "얼마나 자주 지고, **질 때 얼마나 지는가**" 라 전 기간 차이 하나로는 답할 수 없고,
  이 표라야 어느 국면에서 갈라지는지가 보입니다
...
- `daily.csv` 의 **매수 투입액**은 그날 실제로 나간 원화(배정 예산이 아닙니다)이고
  **상한 발동**은 슬롯 상한이 걸린 레벨 수입니다. 사양서 §13.2 의 「일일 최대 투입 비율」과
  「상한 발동 횟수」가 이 둘로 계산됩니다
