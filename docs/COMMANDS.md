...
- **확정 칸을 돕니다.** 목록을 만드는 것은
  `studies/option_expiry/constants.py` 의 `trading_cells()` 이며, 여기에 칸 수를 적지 않습니다 —
  대상이 바뀌면 낡습니다. **같은 값이 본 실행의 `측정.csv` 에도 실립니다** —
  산식의 소유자가 `measure/distribution.py` 하나라 두 값은 언제나 같습니다
...
```bash
# 확정 칸 실행 (기본값) — SPY 9월 아래 · DIA 9월 아래 · DIA 12월 위
# 「만기일 매수 → 다음주 금요일 매도」 매매를 재고 해석 재료를 한 장으로 낸다
poetry run python scripts/run_option_expiry.py

# 종목 하나만 (spy · dia)
poetry run python scripts/run_option_expiry.py --dataset dia
...
# 손절선 격자 전체 (무손절 + -1.0%~-10.0%) — 시세를 재수집한 뒤 손절선을 다시 잴 때만
poetry run python scripts/run_option_expiry.py --stop-grid
```

🔴 **기본 실행이 «확정 칸»입니다** (2026-09-21 사용자 확정). 전에는 대상 5 × 만기월 12 ×
방향 2 = 120칸을 냈고 성적표가 12,000행이었습니다 — 사용자가 실제로 거는 것은 **3칸**이라
그 셋만 냅니다. **왜 그 칸인지는 코드에 없고**
[매매/옵션_만기일/규칙.md](매매/옵션_만기일/규칙.md) §3 이 「확정 / 탈락안 / 근거」로 갖습니다.
그 예외가 언제 허용되는지는 `.claude/rules/trading.md` 가 SoT입니다.

- **가장 먼저 볼 것은 `성적표.csv` 의 `1차 판정` 컬럼**입니다 — **2026-09-16 에 판정표를
  없애고 판정을 성적표로 합쳤습니다.** 기본 실행은 확정 손절선 한 종이라 `시기 = 전체` 로만
  거르면 됩니다. 🔴 **`손절선(%) = 무손절` 로 거르면 0행입니다** — 그 행은 `--stop-grid` 에만
  있고, 맨몸과의 대조 수치는 [매매/옵션_만기일/규칙.md](매매/옵션_만기일/규칙.md) §2.6 이 갖습니다
- **화면에는 게이트를 넘은 칸만, 성적표에는 전 칸이** 남습니다 — **제외된 칸도 값이 그대로 있습니다**
- **`손익분기 대비(%p)` 가 `손익분기 승률(%)` 바로 옆에 있습니다** (2026-09-21) —
  승률에서 손익분기를 뺀 값이며 **표에 실린 반올림 뒤 값끼리** 뺀 것입니다.
  음수면 손익분기에 못 미친다는 뜻이고, 그래서 이름이 「여유」가 아닙니다
- **판정은 게이트 하나뿐이고 조건도 하나입니다** (2026-09-17) — **회당 기대값**의 하한이며,
  **값은 이 문서에 적지 않습니다.** `src/verify_lab/measure/screening.py` 의
  `MIN_EXPECTED_VALUE` 가 SoT 이고 **문서는 이름만 가리킵니다** —
  숫자를 옮겨 적으면 값이 바뀔 때 조용히 낡습니다 (`.claude/rules/trading.md`).
  **적중률 하한은 걷어냈습니다** — `승률(%)` 은 표에 그대로 나오고 그것으로 거를지는 사용자가 정합니다.
  **사용자가 정한 최소치이지 거래비용 문턱이 아닙니다** — 성적은 맨몸입니다.
  **등급은 2026-09-12 에 없앴습니다** — 근거는 루트 [CLAUDE.md](../CLAUDE.md) 「후보 판정 기준」입니다
- **「최악」이 두 컬럼입니다** — `최악(%)`(팔 때)과 `보유 중 최악(%)`(끌고 가는 동안).
  결과만 보면 같아 보이는 칸이 실재하므로 **둘을 나란히 봅니다** (`.claude/rules/trading.md`)

- 선행 조건: `storage/market/` 에 **SPY·DIA 의 원본가와 수정주가 네 파일**이 있어야 합니다 —
  수정주가는 배당락을 재는 데 쓰며, 없으면 실행이 거부됩니다
- **청산 요일 축은 2026-09-16 에 없앴습니다** — 만기가 목요일인 시장에서는 같은 「다음주 금요일」이
  6거래일이 되지만, 요일에 따라 판정이 갈리는 칸이 5개였고 방향이 제각각이라 축을 없앴습니다
...
| 파일 | 무엇 | 행 수 |
| --- | --- | ---: |
| `성적표.csv` | 칸 × 손절선 × 시기의 체결 성적과 `1차 판정` | 15 |
| `거래내역.csv` | 체결 원자료 — 차트로 대조하는 자리 | 89 |
| `측정.csv` | **해석 재료** — 중앙값 · 기준선 · 우연확률 · 평균-비율 어긋남 · 배당락 | 3 |

**행 수는 확정 3칸 · 확정 손절선 한 종 · 시기 5행 기준입니다.** `summary.json` 이 함께 남습니다.
**`--stop-grid` 로 돌리면 성적표 300행 · 거래내역 1,780행이 됩니다.**

- 🔴 **`측정.csv` 의 값은 «1배 롱 기준»입니다.** 「아래」 칸의 평균은 음수로 나오고
  **성적표의 같은 칸과 부호가 다릅니다** — 둘은 대조 대상이 아닙니다. 부호를 뒤집어 저장하지
  않는 것은 그러면 `기준선 오른 비율` 이 실제로는 내린 비율을 가리켜 이름이 거짓이 되기 때문입니다
- **측정 표 여덟을 한 장으로 합쳤습니다** (2026-09-21). 없앤 것과 그 사정은
  `studies/option_expiry/constants.py` 의 `OUTPUT_FILES` 주석이 표로 갖습니다 —
  상대 거래일 ±10 격자는 **결과 문서가 「우위 없음」으로 닫은 축**이고, 보유 거래일 축은
  판정에 쓰이지 않았으며, 시기 2분할은 성적표의 시기 5행과 답이 겹쳤습니다
- **배당락 세 컬럼이 `측정.csv` 에 있습니다** — `대조 건수` · `걸린 건수` · `평균 왜곡(%p)`.
  **대조 건수가 신호 수보다 적으면 수정주가가 그 구간을 못 덮은 것**이며, 「걸린 건 0」과
  「못 쟀다」는 다른 사실입니다. 같은 값을 `check_expiry_dividend.py` 로도 확인할 수 있습니다 —
  산식의 소유자가 `measure/distribution.py` 하나라 두 값은 언제나 같습니다
- **CSV 컬럼 헤더는 한글**이고 비율은 백분율로 저장됩니다
- **성적표는 구간별로 나옵니다** — `전체 · 앞 절반 · 뒤 절반 · 최근 10년 · 최근 5년`.
  **표본이 10건 미만인 구간도 행이 남고** `판정가능` 이 `아니오` 로 찍힙니다
  (루트 [CLAUDE.md](../CLAUDE.md) 측정의 원칙 17). **게이트는 `시기 = 전체` 행에만 걸리고**
  나머지 넷은 관찰용입니다
- **「최근 N년」은 데이터 마지막 거래일 기준**입니다. 실행 시각과 무관하므로 같은 데이터면 같은 결과입니다
- **종목 컬럼은 종목코드가 아니라 종목명입니다.** 코드는 `summary.json` 의 `datasets` 에 있습니다

#### 체결 — 확정 손절선 하나 (격자는 스위치)

- 🔴 **기본은 확정 손절선 −5% 한 종입니다** (2026-09-21 사용자 확정). 성적표 **15행** ·
  거래내역 **89행**입니다. 확정값의 SoT 는 `studies/option_expiry/constants.py` 의
  `EXPIRY_STOP_LEVEL` 이고 고른 근거는 `매매/옵션_만기일/설계.md` 결정 ㊴ 입니다
- **격자는 `--stop-grid` 로 켭니다** — 무손절 + −1.0%~−10.0%(0.5%p 간격). 성적표 300행 ·
  거래내역 1,780행이 되며, **시세를 재수집한 뒤 손절선을 다시 잴 때만** 씁니다.
  **손절선 «값»은 여전히 인자가 아닙니다** — 스위치는 격자를 전부 켜는 것이지
  값을 고르게 해 주지 않습니다
- 🔴 **`--stop-grid` 는 기본 산출물을 «덮습니다».** 산출물 폴더가 매매법당 하나이고 쓰기 전에
  비워지므로, 한 번 돌리면 git 이 추적하는 세 파일이 격자 산출물로 바뀌어 **diff 에 285행 ·
  1,691행 추가로 잡힙니다.** 값이 바뀐 것이 아니며 **인자 없이 다시 돌리면 복원됩니다**
- **사라진 무손절 대조축의 값은 [매매/옵션_만기일/규칙.md](매매/옵션_만기일/규칙.md) §2.6 이 갖습니다.**
  🔴 **§2.5 는 KODEX 200 을 포함한 5칸 기준이라 결론이 반대입니다**(손절이 합계를 깎음).
  인용할 때 어느 구성인지 반드시 확인하십시오 — **수치는 여기 옮겨 적지 않습니다**(같은 숫자가
  여러 곳에 복제되면 한쪽만 낡고 어느 쪽이 현재인지 판별할 방법이 사라집니다)
- 🔴 **「후보가 가장 많아지는 손절선」을 고르지 마십시오.** 선정 기준은
  [매매/옵션_만기일/규칙.md](매매/옵션_만기일/규칙.md) §3.1 의 「평평한 구간 + 최악 통제」입니다.
  **그 격자는 축소 전 실행(후보 24칸 · 체결 612건)으로 잰 것이라 지금 구성으로는 재현되지 않습니다** —
  값 −5% 는 재선정하지 않습니다
- **`손절선(%)` 컬럼은 전 행이 같은 값이어도 나옵니다** — 없으면 그 표가 무손절 성적인지
  −5% 성적인지 산출물만 봐서는 판별되지 않습니다
- **청산은 다음 주 «금요일» 하나입니다.** 산출물에 `청산 요일` 컬럼이 없습니다 —
  근거는 `매매/옵션_만기일/설계.md` 결정 ⑳ 입니다
- **9월 두 칸(SPY·DIA)은 같은 날 같은 방향**이고 **상관 0.965 로 28건 내내 부호가 갈린 적이
  없습니다** — 독립된 두 번의 기회가 아니며 투자금 결정이 그 사실을 봅니다
...
```bash
# 기본 실행 — 확정 칸(9월 아래) · 확정 손절선 하나. 측정 8대상 + 체결 6대상
...
# 손절선을 다시 골라야 할 때 — 격자를 «전부» 켭니다 (무손절 대조축 포함)
poetry run python scripts/run_month_end.py --stop-grid

# 무작위 뽑기 대조의 반복 수·시드 (기본값 1000 / 0)
poetry run python scripts/run_month_end.py --repeats 2000 --seed 1
```

- 🔴 **기본 실행은 확정 칸 하나와 확정 손절선 하나만 냅니다** (2026-09-21 사용자 결정).
  성적표 2,640 → **30행** · 거래내역 10,648 → **161행** · 측정 표 8개 → **`측정.csv` 1개**입니다.
  **왜 이 칸인지는 `docs/매매/월말_진입/규칙.md` §3 이 「확정 / 탈락안 / 근거」로 갖습니다** —
  목록만 보고 근거를 짐작하면 안 됩니다
- 🔴 **`--stop-grid` 는 격자를 «전부» 켜는 스위치일 뿐 값을 고르게 해 주지 않습니다.**
  손절선 값을 인자로 받으면 그것은 `.claude/rules/trading.md` 가 금지한
  「성과가 좋아지는 값을 찾아 돌리기」입니다
- 🔴 **시세를 재수집하면 확정 칸 목록을 다시 판단해야 합니다.** 성적이 바뀌는데 코드의 목록은
  따라오지 않습니다 — 그것이 격자를 내던 이유였고 지금도 사실입니다
- 산출물은 `storage/results/매매/월말_진입/` 에 **CSV 셋**과 `summary.json` 으로 남습니다
  - `측정.csv` — 확정 칸마다 한 행. **판정에 쓰지 않지만 해석에 필요한 값**(중앙값·평균-비율
    어긋남·기준선·우연확률·배당락)이며 **성적표에는 그중 어느 것도 없습니다.**
    🔴 **값은 1배 롱 기준 그대로**라 「아래」 칸이어도 부호가 뒤집히지 않습니다 —
    성적표의 평균과 부호가 다르고 **대조 대상이 아닙니다**
  - `성적표.csv` — 종목 × 월 × 방향 × 손절선 × 시기. `1차 판정` 이 그 안에 있습니다
  - `거래내역.csv` — 체결 원자료. **청산가는 실제 체결가**라 손절이 걸린 건은 손절가(또는 갭 시가)입니다
- 🔴 **측정 기본은 8대상, 체결 기본은 6대상입니다.** 체결은 **인버스 둘(`114800`·`251340`)을
  뺀 것**이며 — 1배의 부호를 뒤집은 값과 차이가 0.01~0.08%p 로 잡음이고 실제 집행은
  2배 인버스입니다. **측정에는 그대로 둡니다** — 어긋나는 칸이 곧 분배락이 걸린 칸이라
  확정 전 교차검증에 씁니다. **체결에서도 `--ticker 114800` 로 지목하면 돕니다**
- **게이트 판정을 받는 것은 살 수 있는 1배 ETF 둘뿐입니다** — 지수 넷은 `1차 판정` 이
  「판정 안 함」이며 긴 시계열을 참고하는 용도입니다 (루트 `CLAUDE.md` 「후보 판정 기준」)
- **지수도 받습니다. 다만 손절선이 한 줄로 강등됩니다.** 장중 손절에 고가·저가가 필요한데
...
- 🔴 **폴더 이름은 대상 범위도 잰 구간도 말하지 않습니다.** 구분자는 `summary.json` 의
  `datasets`(범위)와 `rule`(확정 칸·손절선·시작 연도)뿐입니다
- **맨몸 성적입니다** — 수수료·슬리피지·세금을 넣지 않습니다
  (루트 [CLAUDE.md](../CLAUDE.md) 2026-09-06 확정)
- **종목 컬럼은 종목코드가 아니라 종목명입니다.** 코드는 `summary.json` 의 `datasets` 에 있습니다
- **「시기 시작일」·「시기 종료일」 컬럼이 있습니다** — 「앞 절반」이 대상마다 다른 기간을 뜻하기
  때문입니다 (코스피 종합 1980~2002 대 KODEX 코스닥150 2016~2020)
- **성적표는 구간별로 나옵니다** — `전체 · 앞 절반 · 뒤 절반 · 최근 10년 · 최근 5년`.
  표본이 10건 미만인 구간도 행이 남고 `판정가능` 이 `아니오` 로 찍힙니다
- **난수는 `측정.csv` 의 우연확률에만 쓰입니다.** 체결 성적에는 쓰이지 않아 같은 데이터면
  성적표와 거래내역이 언제나 같습니다. 실행 시간은 수 초입니다
- 선행 조건은 **ETF 네 파일과 지수 네 파일**입니다. 지수는 위 「지수 수집」으로 받습니다
- 규칙과 성적은 `docs/매매/월말_진입/규칙.md`, 신호의 통계적 근거는
  `docs/매매/월말_진입/결과.md`, 확정 설계는
  [매매/월말_진입/설계.md](매매/월말_진입/설계.md) 입니다

#### 월말 확정 칸의 배수별 실물 성적

확정 칸을 **1배 롱(부호 뒤집기) · 1배 실물 · 2배 실물**로 나란히 재고 손절선 격자를 전부 냅니다.
**실집행은 2배 인버스인데 성적표는 1배 기준**이라 그 환산이 맞는지 보는 자리입니다.

```bash
poetry run python scripts/data/check_month_end_leverage.py
```

- **외부 서버에 요청하지 않습니다.** 여섯 종목의 원본가 파일이 있어야 합니다
- 🔴 **손절선을 «기초자산» 기준으로 환산합니다** — 1배 −5% 는 2배 상품에서 −10% 입니다.
  상품 가격 기준으로 옮기면 기초자산 −2.5% 에서 자르는 훨씬 타이트한 규칙이 됩니다
- **이 성적의 자리는 산출물이 아니라 `docs/매매/월말_진입/규칙.md` §2 입니다** —
  2배 상품은 판정 표본이 아니라 성적표에서 빠졌기 때문이며, **재수집하면 이것을 다시 돌려
  그 절을 고칩니다**
- **코스닥 −2배는 ETF 가 없어 ETN(`530107`)을 씁니다** — 2022-10-17 상장이라 표본이 3건이고,
  **통계가 아니라 사례**입니다

### 검증 — 만기_말일 (진입 2종 × 청산 2종)

옵션 만기일과 월말 진입이 각자 쓰던 **진입**과 **청산**을 한 축에 올려 네 조합을 나란히 잽니다.
**9월·12월**만 재고 대상은 여섯(미국 ETF 둘 · 국내 ETF 둘 · 국내 기초지수 둘)입니다.

```bash
# 기본 실행 — 네 조합 × 두 달 × 여섯 대상
...
# 조합을 골라서 — c1=만기→다음주금 · c2=만기→말일 · c3=20일→다음주금 · c4=20일→말일
poetry run python scripts/run_expiry_monthend.py --combo c1 --combo c2

# 달을 골라서 (기본값 9·12)
poetry run python scripts/run_expiry_monthend.py --month 9

# 무작위 뽑기 대조의 반복 수·시드 (기본값 1000 / 0)
poetry run python scripts/run_expiry_monthend.py --repeats 2000 --seed 1
```

- **산출물은 `storage/results/검증/만기_말일/` 에 CSV 셋**입니다 — `통계.csv` 48행 ·
  `성적표.csv` 800행 · `거래내역.csv` 3,872행, 그리고 `summary.json`
- 🔴 **9월·12월은 사후에 고른 달입니다.** 두 매매법이 이미 그 달을 가리킨 뒤에 골랐으므로
  이 성적으로 「우위가 있다」를 새로 주장할 수 없고, **용도는 조합 간 비교**입니다
- 🔴 **네 조합이 절반 가까운 해에 같은 날로 붕괴합니다.** 조합 간 성적 차이가 몇 건에서
  나온 것인지는 `통계.csv` 의 **「이 조합만 다른 해」**가 말합니다 — **그 값은 매 실행 다시
  세므로 여기에 숫자를 적지 않습니다.** 그때의 수치는 `docs/검증/만기_말일/결과.md` §5 에 있습니다 — 한국 12월의 20일 진입 두 조합은
  그 값이 **0** 이라 독립된 관측이 아닙니다
- **손절선 격자 스위치가 없습니다.** ETF 는 확정 −5% 와 무손절 대조 두 종, 지수는 손절불가
  한 종뿐입니다 — 조합을 비교하는 것이 목적이라 손절선까지 축으로 두면 무엇이 차이를
  만들었는지 갈리지 않습니다
- **한국에도 셋째 금요일을 씁니다** — 국내 만기(둘째 목요일)는 이미 재 봤고 성과가 약했습니다.
  앞당김은 `KODEX 200` 47해 중 2해뿐이며 둘 다 추석입니다
- **난수는 `통계.csv` 의 우연확률에만 쓰입니다.** 체결 성적에는 쓰이지 않아 같은 데이터면
  성적표와 거래내역이 언제나 같습니다
- 선행 조건은 **ETF 네 파일**(SPY·DIA·069500·229200)과 **지수 두 파일**(1028·2203)입니다
- 결과와 확정 설계는 `docs/검증/만기_말일/결과.md` 와 `docs/검증/만기_말일/설계.md` 입니다
