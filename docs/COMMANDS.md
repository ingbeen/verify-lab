...
- 산출물은 `storage/results/검증/<실행시각>_reverse/` 에 CSV 4개(`signals`·`statistics`·`excess`·`test`)와
...
- **산출물은 9개 CSV 이고, 가장 먼저 볼 것은 `candidates.csv`** 입니다 — 전 칸의 1차 판정과
  그 판정을 읽는 데 필요한 값(표본·방향·적중률·기대값·합산·기준선)이 들어 있습니다.
  화면에도 실행 직후 그 표가 먼저 나옵니다
- **화면에는 1차 게이트를 넘은 칸만, `candidates.csv` 에는 전 칸이** 남습니다.
  화면은 적중률 내림차순이고 CSV 는 만기월 순서입니다 — **제외된 칸도 값이 그대로 있습니다**
- **판정은 게이트 둘뿐입니다** (적중률 60% 이상 · 방향 기대값 0 초과).
  **등급은 2026-09-12 에 없앴습니다** — 규격의 SoT 는 `src/verify_lab/measure/screening.py` 이고
  루트 [CLAUDE.md](../CLAUDE.md) 「후보 판정 기준」이 그 근거를 적습니다
...
- 산출물은 `storage/results/매매/<실행시각>_reverse/` 에 `거래내역.csv`(체결 내역),
  `성적표.csv`(대상 × 구간 집계), `summary.json` 으로 남습니다
- `거래내역.csv` 는 **신호 하나가 한 행**이고, `성적표.csv` 는 **대상 하나가 구간 5행**입니다
...
- 산출물은 `storage/results/매매/<실행시각>_option_expiry/` 에 남습니다
  - 기본: `성적표.csv`(대상 칸 × 구간) · `거래내역.csv`(체결 원자료) · `summary.json`
  - `--grid`: `손절선_격자.csv`(대상 칸 × 손절선 격자 × 구간) · `거래내역.csv` · `summary.json`
...
- 🔴 **매매 기본 대상은 6개이고 검증은 8개입니다** (2026-09-12). 매매는 코스피 셋
  ...
  (`114800`·`251340`)은 매매 성적표에서만 뺐습니다** — 1배의 부호를 뒤집은 값과 차이가
...
- **`--from-year` 는 «신호»만 거릅니다.** 시세를 자르지 않으므로 「최근 10년/5년」의 기준일이
  두 실행에서 같고, 그래서 두 산출물을 나란히 놓고 읽을 수 있습니다.
  **미리 정한 구간을 대조하는 축이지 성과가 좋아지는 값을 찾는 노브가 아닙니다** —
  쓴 값은 `summary.json` 의 `rule.from_year` 에 남습니다
- 🔴 **폴더 이름은 대상 범위도 잰 구간도 말하지 않습니다.** 두 실행이 같은 slug 를 쓰므로
  구분자는 `summary.json` 의 `datasets`(범위)와 `rule.from_year`(구간)뿐입니다
...
- **손절선 하나로 고정한 표를 보려면 `손절선(%)` 을 거릅니다** — 그 값(권고안은 `-5.0`,
  ...
  **함께** 고르면 여덟 대상이 한 장에 들어가 지수 16~46년과 ETF 10~24년을 나란히 볼 수
  ...
  🔴 **`무손절` 을 같이 고르면 안 됩니다** — 그것은 ETF 의 대조축이라 480행이 섞여 대상이 두 번 실립니다.
  반대로 숫자 값만 고르면 지수 넷이 빠져 긴 기간 축이 사라집니다
- **종목 컬럼은 종목코드가 아니라 종목명입니다.** 코드는 `summary.json` 의 `datasets` 에 있습니다
- **「구간 시작일」·「구간 종료일」 컬럼이 있습니다** — 「앞 절반」이 대상마다 다른 기간을 뜻하기
  때문입니다 (코스피 종합 1980~2003 대 KODEX 코스닥150 2016~2020)
- **성적표는 구간별로 나옵니다** — `전체 · 앞 절반 · 뒤 절반 · 최근 10년 · 최근 5년`.
  표본이 10건 미만인 구간도 행이 남고 `판정가능` 이 `아니오` 로 찍힙니다
