...
- **`storage/series/<지수>_index.csv` 에 종가 하나짜리 계열로 저장합니다.** 시세가 아닙니다 —
  ...
  근거는 [검증/월말_진입/설계.md](검증/월말_진입/설계.md) §7.6·§7.11 에 있습니다
...
- 「아래」로 걸면 원본가 성적이 그만큼 **과대평가**돼 있습니다 — 인버스는 그만큼 오르지 않습니다.
  실측 결과는 `docs/검증/월말_진입/결과.md` §10 에 있습니다
...
- 산출물은 `storage/results/검증/월말_진입/` 에 측정 CSV 6개(`trades`·`grid`·`통계`·
  `month_halves`·`periods`·`execution`)와 체결 산출물(`성적표`·`거래내역`),
  `summary.json` 으로 남습니다
- 🔴 **판정표 둘(`1차_판정.csv`·`grid_candidates.csv`)을 2026-09-16 에 없앴습니다.**
  1차 판정은 `성적표.csv` 의 컬럼이고, **격자 축의 값은 `grid.csv` 616행에 그대로 있습니다** —
  되살리는 절차는 [검증/월말_진입/설계.md](검증/월말_진입/설계.md) §7.17 입니다
...
- 결과와 판정은 `docs/검증/월말_진입/결과.md`, 확정 설계는
  [검증/월말_진입/설계.md](검증/월말_진입/설계.md) 입니다
...
- 🔴 **매매 기본 대상은 6개이고 검증은 8개입니다** (2026-09-12). 매매는 코스피 셋
  ...
  **검증에는 그대로 둡니다** — `execution.csv` 가 「아래」를 **인버스 실물로 재는 표**이고,
  ...
  (`docs/검증/월말_진입/설계.md` §7.16). **매매에서도 `--ticker 114800` 로 지목하면 돕니다**
...
- **지수도 받습니다. 다만 한 줄로 강등됩니다.** 장중 손절에 고가·저가가 필요한데
  지수 네 계열은 종가만 저장돼 있습니다 (`검증/월말_진입/설계.md` §7.6).
...
- 산출물은 `storage/results/검증/월말_진입/` 에 남습니다
...
- **손절선 하나로 고정한 표를 보려면 `손절선(%)` 을 거릅니다** — 그 값(권고안은 `-5.0`,
  `검증/월말_진입/규칙.md` §1 은 **아직 사용자 확정 전**입니다)과 `손절불가` 를
...
- 규칙과 성적은 `docs/검증/월말_진입/규칙.md`, 신호의 통계적 근거는
  `docs/검증/월말_진입/결과.md` 입니다
