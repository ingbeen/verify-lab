...
>   `docs/조사/투자금_결정/규칙.md` §1.2 의 계산기 입력이 여기서 옵니다
...
### 매매 — 만기_말일 (9월 만기일 매수 → 말일 매도, 아래)

**기본 실행은 확정 칸만 냅니다** — 9월 · 「셋째 금요일 종가 매수 → 그 달 마지막 거래일 종가
매도」 · **아래** 방향 · SPY · DIA · KODEX 코스닥150 (+ 코스닥150 지수는 참고, 판정 안 함).

```bash
# 기본 실행 — 확정 칸 넷
poetry run python scripts/run_expiry_monthend.py

# 대상을 넓혀서 (여러 번 줄 수 있습니다. 전체는 SPY·DIA·069500·1028·229200·2203)
poetry run python scripts/run_expiry_monthend.py --ticker SPY --ticker 069500

# 조합을 넓혀서 — c1=만기→다음주금 · c2=만기→말일(확정) · c3=20일→다음주금 · c4=20일→말일
poetry run python scripts/run_expiry_monthend.py --combo c1 --combo c2

# 달을 넓혀서 (확정은 9월, 잰 적 있는 달은 9·12)
poetry run python scripts/run_expiry_monthend.py --month 9 --month 12

# 무작위 뽑기 대조의 반복 수·시드 (기본값 1000 / 0)
poetry run python scripts/run_expiry_monthend.py --repeats 2000 --seed 1
```

- **산출물은 `storage/results/매매/만기_말일/` 에 CSV 셋**입니다 — `측정.csv` 4행 ·
  `성적표.csv` 35행 · `거래내역.csv` 158행, 그리고 `summary.json`
- **확정 규칙은 `docs/매매/만기_말일/규칙.md` 입니다.** 9월에 매매하기 전에 §1 만 읽으면 됩니다
- 🔴 **확정 칸의 근거는 `규칙.md` §3 이 갖습니다.** 목록만 보고 근거를 짐작하면 사후 선택과
  구별되지 않습니다. **시세를 재수집하면 그 목록을 다시 판단합니다** — 그때 위 세 인자로 넓혀 돌립니다
- 🔴 **9월은 사후에 고른 달입니다.** 두 매매법이 이미 그 달을 가리킨 뒤에 골랐으므로
  이 성적으로 「우위가 있다」를 새로 주장할 수 없습니다
- 🔴 **조합을 하나만 돌리면 「다른 조합과 같은 해」·「이 조합만 다른 해」가 빕니다.**
  견줄 조합이 없어 **잴 수 없기** 때문이며, 채우면 신호 수와 같아져 거짓이 됩니다 —
  실측으로 SPY 9월은 넷을 다 돌리면 **15해**이고 하나만 돌리면 33 으로 나옵니다.
  그때의 수치는 `docs/매매/만기_말일/결과.md` §5 에 있습니다
- **방향은 「아래」로 고정입니다.** 반대 방향을 격자로 내지 않으며, 방향을 다시 고를 재료는
  `측정.csv` 의 **오른 비율·내린 비율**(1배 롱 기준)이 그대로 줍니다
- **손절선 격자 스위치가 없습니다.** ETF 는 확정 −5% 와 무손절 대조 두 종, 지수는 손절불가
  한 종뿐입니다 — 손절선은 두 매매법이 이미 확정한 값이고 재선정하지 않습니다
- **한국에도 셋째 금요일을 씁니다** — 국내 만기(둘째 목요일)는 이미 재 봤고 성과가 약했습니다.
  앞당김은 `KODEX 200` 47해 중 2해뿐이며 둘 다 추석입니다
- **난수는 `측정.csv` 의 우연확률에만 쓰입니다.** 체결 성적에는 쓰이지 않아 같은 데이터면
  성적표와 거래내역이 언제나 같습니다
...
### 검증 — 중간선거_사이클 (9월 마지막 거래일 매수 → 다음해 6월 마지막 거래일 매도, 위)

**측정과 체결을 한 번에 돕니다.** 산출물은 `storage/results/검증/중간선거_사이클/` 에
`측정.csv`·`거래내역.csv`·`성적표.csv`·`분기.csv`·`분기내역.csv`·`summary.json` 여섯으로 나옵니다.
...
- **축은 「중간선거해」 하나입니다** (2026-09-22 사용자 결정). 사이클 네 칸을 견주어
  ...
  **10월 첫 거래일 진입으로 잰 기록**이며 `docs/검증/중간선거_사이클/결과.md` §17.1 에 있습니다 —
  **지금 구성으로는 재현되지 않습니다**
...
**두 매매법이 답하던 질문은 대부분 `만기_말일` 이 받는다** — 진입 2종 × 청산 2종이 축으로
남아 있어 `--combo`·`--month` 로 넓히면 같은 자리를 잰다. **배당락은 `측정.csv` 의
세 컬럼**이 매 실행 낸다.
