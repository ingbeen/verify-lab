...
- 산출물은 `storage/results/매매/역방향/` 에 CSV **5개**(`signals`·`통계`·`excess`·`test`·`1차_판정`)와
...
- 🔴 **`1차_판정.csv` 는 화면에 나오지 않습니다** (2026-09-12 신설). 축은 **구간(보유일)**이고
  전 칸이 그대로 들어 있습니다. **기준선 열은 없습니다** — 판정이 묻지 않는 축이라 뺐고
  그 값은 `통계.csv` 가 담습니다. **후보만 뽑아 화면에 찍지 않는 것이
...
- **산출물은 9개 CSV 이고, 가장 먼저 볼 것은 `1차_판정.csv`** 입니다 — 전 칸의 1차 판정과
  그 판정을 읽는 데 필요한 값(표본·방향·적중률·기대값·합산·기준선)이 들어 있습니다.
  화면에도 실행 직후 그 표가 먼저 나옵니다
- **화면에는 1차 게이트를 넘은 칸만, `1차_판정.csv` 에는 전 칸이** 남습니다.
...
- 산출물은 `storage/results/매매/옵션_만기일/` 에 9개 CSV 와 `summary.json` 으로 남습니다.
...
- 대상은 **전 칸**(대상 × 만기월 12 × 방향 2)이며 목록을 만드는 것은
  `studies/option_expiry/constants.py` 의 `all_cells()` 입니다
...
- 산출물은 `storage/results/매매/옵션_만기일/` 에 남습니다
  ...
  - 그 옆에 **측정 표 9개**가 함께 있습니다 — 가장 먼저 볼 것은 `1차_판정.csv` 입니다
...
- **성적표의 무손절 행이 판정표(`1차_판정.csv`)의 방향 기대값과 맞는지** 확인하세요.
...
- 산출물은 `storage/results/검증/월말_진입/` 에 CSV 8개(`trades`·`grid`·`months`·
  `month_halves`·`periods`·`grid_candidates`·`1차_판정`·`execution`)와
