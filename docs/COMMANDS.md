...
```bash
# ETF 둘 + 지수 둘 전부 (기본값) — 12개월 × 두 방향 × (손절선 8종 + 무손절)
...
- **지수도 받습니다. 다만 무손절 한 줄로 강등됩니다.** 장중 손절에 고가·저가가 필요한데
  코스닥 지수 두 계열은 종가만 저장돼 있습니다 (`spec/월말_진입_설계.md` §7.6).
...
- 산출물은 `storage/results/매매/<실행시각>_month_end/` 에 남습니다
  ...
  - `성적표_고정손절.csv` — **손절선을 −5% 로 고정해 그 행만 고른 표.**
    네 대상이 한 장에 들어가 지수 30년과 ETF 11년을 나란히 볼 수 있습니다.
    **격자에서 골라낸 행이지 다시 계산한 값이 아닙니다.**
    컬럼 구성은 `성적표.csv` 와 같습니다 — **`손절선(%)` 을 남깁니다.** ETF 행은 `-5.0`,
    지수 행은 `무손절` 이라 **두 행이 서로 다른 규칙으로 만들어진 성적**이기 때문입니다
  - `summary.json` — 손절선 목록, **고정 표가 쓴 손절선**, 대상별 기간·진입 건수, **비용 미반영 표기**
- **종목 컬럼은 종목코드가 아니라 종목명입니다.** 코드는 `summary.json` 의 `datasets` 에 있습니다
- **「구간 시작일」·「구간 종료일」 컬럼이 있습니다** — 「앞 절반」이 대상마다 다른 기간을 뜻하기
  때문입니다 (코스닥 종합 1996~2010 대 KODEX 코스닥150 2016~2020)
- **성적표는 구간별로 나옵니다** — `전체 · 앞 절반 · 뒤 절반 · 최근 10년 · 최근 5년`.
  표본이 10건 미만인 구간도 행이 남고 `판정가능` 이 `아니오` 로 찍힙니다
