...
- **매매 계층은 매매법마다 `<slug>_runner.py` 하나**입니다. 체결 판정식(`strategy/trade_fill.py`)과
  시기별 성적 산식(`strategy/periods.py`)은 **매매법 이름을 갖지 않는 공유 모듈**이 소유하며,
...
- **폴더 이름이 대상 범위를 말하지 않습니다.** 같은 slug 를 측정과 매매가 공유하므로 범위가
  갈릴 수 있습니다 — 월말은 **매매가 검증보다 좁습니다**(인버스가 매매 기본에서 빠지며 지목하면 돕니다).
  **여기에 구체적인 대상 수를 적지 않습니다** — 범위의 SoT 는 산출물 `summary.json` 의 `datasets` 이고,
  코드에서는 `studies/month_end/constants.py` 의 `DATASETS`·`DATASETS_TRADING` 입니다
...
| [spec/월말_진입_설계.md](spec/월말_진입_설계.md) | **검증 #10 확정 설계** — 월 하순 진입(20일 매수 → 말일 매도), **코스피·코스닥 8대상. 매매 기본값만 여섯**(인버스는 매매에서만 제외 §7.16). 진입일 앞당김과 말일 기준 상대 거래일 청산, 진입 11칸 × 청산 7칸 격자, **집행 축**(「아래」를 인버스 실물로 재는 표 — 기본 실행 대상 변경의 영향은 §7.16), 확정된 설계 결정, 데이터 실측 기록(**데이터 마지막 달이 가짜 표본을 만드는 함정**·격자 극단 칸의 표본 불균등·보유 거래일 분포·**네 지수의 조회 구간과 소급 산출 0값**) | 검증 #10 관련 전부. **부분 월 함정은 §7.1**, 코스피 지수 실측은 **§7.11** |
...
| [research/역방향.md](research/역방향.md) | **검증 #1 결과** — 역대급 등락 이후 수익률 측정과 판정. **후보 판정은 이 문서에 없다** — 2026-09-12 에 검증 코드가 `candidates.csv` 를 내기 시작했고(`spec/역방향_설계.md` §7 결정 ㉖) **그 판정 결과는 `strategy/역방향_매매_규칙.md` §2.4 가 담는다.** 이 문서는 그 이전 측정이라 현재 코드로 재현되지 않는다 | 검증 #1 결과를 인용하거나 재실행할 때 |
...
| [strategy/옵션_만기일_매매_규칙.md](strategy/옵션_만기일_매매_규칙.md) | **옵션 만기일 매매의 SoT** — 확정 규칙(§1)·시기별 성적(§2)·결정 근거(§3)·한계(§4)·원자료(§5). **집행은 4칸**(9월 둘·12월 둘, **전부 미국 ETF**), 손절 −5%, **알림 없음(직접 챙김)**. **기본 실행은 그보다 넓은 `EXPIRY_CELLS` 전 칸을 냅니다**(§1.1). 제외한 칸이 둘이고 근거가 다르다 — **QQQ 두 칸은 배당락이 보유 구간에 걸려**(§3.2·§3.4, 데이터 오염), **KODEX 200 9월은 근거가 약해**(§3.2, 손익비 0.788 · t=1.16 · 최악 1건이 22년 성과의 33%) 뺐다. §2 의 성적표는 KODEX 를 포함한 5칸 기준이며 **측정 기록으로 남겨 둔 것**이다 | **9월·12월에 매매하기 전에 §1 만 읽으면 된다.** 2026년 실행 날짜는 §1.6. **손절선 격자 전체는 §3.1.** 국내 칸을 다시 넣으려면 §1.4 의 릴레이 근거를 본다 |
| [strategy/월말_진입_매매_규칙.md](strategy/월말_진입_매매_규칙.md) | **월말 매매의 손절 격자 — 코스피·코스닥 6대상** (2026-09-12 에 인버스 2개를 «매매» 기본에서 뺐다 §2.2). 20일 매수 → 말일 매도에 손절선 −3~−10%(1%p 간격)와 무손절을 걸어 12개월 × 두 방향으로 냈다. **−5% 이상이면 후보 달에서 손절이 거의 걸리지 않아 무손절과 성적이 같고**(6·9·12월 0건), **−3%는 반등 구간을 끊어 6월 −8.19%p·12월 −10.77%p 를 깎는다**(§2.1·§3①). 손절이 최악을 **악화시킬 수 있다는 실측**은 §3②. **6대상 매트릭스는 §2.7** — **게이트를 받는 것은 살 수 있는 1배 ETF 둘뿐**이고(지수는 참고 · 인버스는 기본 제외 §2.2) KODEX 코스닥150 이 일곱 달, KODEX 200 이 다섯 달을 넘는다. **2000년 이후로 잘라도 12월은 살아나지 않는다**(§2.8, 셋 다 더 약해진다). §1 은 대상 달을 **9·12월**로 좁힌 권고안이며 **4월은 분배락으로 뺐고 5월은 미결**이다. 손절선도 사용자 확정 전이다 | 손절선이나 대상 달을 정하거나 그 근거를 인용할 때. **12개월 전체에서 손절이 무엇을 했는지는 §2.4**, **5월을 왜 미결로 뒀는지는 §1** |
...
| `storage/results/{검증, 매매, 실측}/<매매법>/` | 산출물 (CSV, summary.json). **계층이 경로로, 매매법이 폴더 이름으로 드러납니다** | 동기화 |
...
`--grid` 처럼 추가 파일을 내는 옵션이 있어 이 비우기가 필요합니다.
