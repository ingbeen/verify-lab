...
| [spec/usdkrw_grid.md](spec/usdkrw_grid.md) | **원달러 그리드 트랙의 SoT** — 데이터 실측 기록 11개 절(ECOS/FRED 코드, ETF 2종, 매매기준율 시차, 동적 범위, 슬롯 상한, 환전 비용, 그리드 엔진 실행 4회 — 비용 없음·거래비용·이자·ETF 경로)과 확정된 설계 결정(A1~A4·B1~B2·C1~C77) | 원달러 그리드·검증 #5 관련 전부. **§4 가 원본 사양서를 이긴다** |
