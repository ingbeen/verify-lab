...
| [research/역방향.md](research/역방향.md) | **검증 #1 결과** — 역대급 등락 이후 수익률 측정과 판정. **후보 판정은 이 검증의 산출물에 없다** — `screen_candidates` 를 부르지 않으므로 그 수치는 재현되지 않는다. **게이트를 소급 적용하는 것은 별건 계획서다**(2026-09-12 사용자 결정) | 검증 #1 결과를 인용하거나 재실행할 때 |
