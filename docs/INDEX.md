...
| [spec/kosdaq_month_end.md](spec/kosdaq_month_end.md) | **검증 #10 확정 설계** — 코스닥 월 하순 진입(20일 매수 → 말일 매도). 진입일 앞당김과 말일 기준 상대 거래일 청산, 진입 11칸 × 청산 7칸 격자, 확정된 설계 결정, 데이터 실측 기록(**데이터 마지막 달이 가짜 표본을 만드는 함정**·격자 극단 칸의 표본 불균등·보유 거래일 분포·코스닥 지수 조회 구간) | 검증 #10 관련 전부. **부분 월 함정은 §7.1** |
...
| [research/코스닥_월말_진입.md](research/코스닥_월말_진입.md) | **검증 #10 결과** — 「20일 매수 → 말일 매도」에 통계적 우위가 있는가. 판정은 **우위 없음**(적중률 46~53%, 1차 게이트를 하나도 못 넘는다). **20일이라는 날짜에도 실체가 없다** — 15~22일이 다 비슷하고 23일부터 뒤집힌다(§6). 격자 308칸의 유의 칸 15개가 우연 기대치 15.4개와 같다. **9월만 네 대상 모두 아래로 기울지만**(적중률 70~80%) 월별 48칸에서 우연확률 0.05 미만이 **0칸**이고 사후 선택 오염을 안고 있다(§7·§10) | 코스닥 월말 매매를 다시 검토하거나 그 수치를 인용할 때. **9월 가설의 한계는 §10** |
