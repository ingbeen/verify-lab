...
| [research/역방향.md](research/역방향.md) | **검증 #1 결과** — 역대급 등락 이후 수익률 측정과 판정. 사용자 판정 기준(적중률 60% · 방향 기대값 0 초과)을 역방향 전체 192칸 중 **109칸(56.8%)** 이 통과한다 | 검증 #1 결과를 인용하거나 재실행할 때 |
...
| `storage/results/<실행시각>_<매매법>/` | 계층 폴더를 도입하기 전의 산출물. **옮기지 않습니다** | 동기화 |
| `storage/results/meta.json` | 실행 이력 (최근 5개 순환) | 제외 (실행마다 재작성돼 PC 간 충돌) |

**산출물이 사는 자리가 둘이고 그것이 의도입니다.** 옛 산출물을 옮기면 그 이름을 인용한
살아있는 문서가 전부 깨지므로, 새 규약은 **앞으로 나오는 산출물부터** 적용합니다.
`src/verify_lab/utils/result_citations.py` 가 두 자리를 모두 훑으며, 같은 이름이 두 자리에 있으면
예외를 던집니다 — 문서가 가리키는 것이 어느 쪽인지 판별할 수 없기 때문입니다.
...
그리고 **옛 산출물 폴더의 파일명은 바뀌지 않습니다** — 디스크에 이미 그 이름으로 있고,
인용이 남아 있는 폴더는 지울 수 없습니다(`tests/test_result_citations.py`). 그래서 살아있는
문서에 두 이름이 함께 보일 수 있습니다.
...
| 별도 실행이라 대체되지 않았다 (예: 손절선 격자) | **옛 폴더·옛 파일명 그대로 둔다.** 그 사실을 문서 머리말에 적는다 |
