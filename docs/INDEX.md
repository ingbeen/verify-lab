...
| [strategy/옵션_만기일_매매_규칙.md](strategy/옵션_만기일_매매_규칙.md) | **옵션 만기일 매매의 SoT** — 확정 규칙(§1)·구간별 성적(§2)·결정 근거(§3)·한계(§4)·원자료(§5). **집행은 4칸**(9월 둘·12월 둘, **전부 미국 ETF**), 손절 −5%, **알림 없음(직접 챙김)**. **기본 실행은 그보다 넓은 `EXPIRY_CELLS` 전 칸을 냅니다**(§1.1). 제외한 칸이 둘이고 근거가 다르다 — **QQQ 두 칸은 배당락이 보유 구간에 걸려**(§3.2·§3.4, 데이터 오염), **KODEX 200 9월은 근거가 약해**(§3.2, 손익비 0.788 · t=1.16 · 최악 1건이 22년 성과의 33%) 뺐다. §2 의 성적표는 KODEX 를 포함한 5칸 기준이며 **측정 기록으로 남겨 둔 것**이다 | **9월·12월에 매매하기 전에 §1 만 읽으면 된다.** 2026년 실행 날짜는 §1.6. **손절선 격자 전체는 §3.1.** 국내 칸을 다시 넣으려면 §1.4 의 릴레이 근거를 본다 |
...
24컬럼이라, 같은 이름을 붙이면 없는 축(`손익비`·`질 때 표본`·`구간 시작일`)이 있는 것처럼 읽힙니다.
