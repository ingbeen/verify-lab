...
상위 폴더가 둘을 가릅니다. 접미사를 계층마다 붙이면 같은 매매법이 `index_extreme` 과
`reverse_trading` 두 이름으로 불려, 읽는 사람이 그 둘을 옛것/새것으로 오해합니다.
...
24컬럼이라, 같은 이름을 붙이면 없는 축(`손익비`·`질 때 표본`·`시기 시작일`)이 있는 것처럼 읽힙니다.
