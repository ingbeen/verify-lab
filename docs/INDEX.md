...
| [strategy/투자금_결정.md](strategy/투자금_결정.md) | **「얼마 넣는가」의 SoT — 매매법에 종속되지 않는다.** 계산식(§1)·검증(§2)·결정 근거(§3)·한계(§4)·재현 절차(§5). `명목 노출 = 켈리분수 × 평균 ÷ 표준편차²` 이고 **켈리분수 1/20 은 매매법이 바뀌어도 고정**한다 — 풀 켈리가 222~3,000% 로 벌어지는 다섯 매매법이 같은 낙폭 2~5% 를 냈다(§2.1). **승률·손익비·표본 수·최악 1건·손절폭을 크기 결정에 넣지 않은 이유**가 각각 §3.2~§3.6 에 있다 | **새 매매법의 투자금을 정할 때 §1 만 읽으면 된다.** 입력은 성적표의 `평균(%)`·`표준편차(%)` 와 손절폭 셋뿐. **켈리분수를 바꾸려 할 때는 §2.2 조견표** |
