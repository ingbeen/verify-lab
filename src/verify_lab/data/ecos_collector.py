...
`docs/조사/원달러_그리드/설계.md` "데이터 실측 기록" 에 있다. 이 값들은 ECOS 가 개편되면 바뀔 수 있으므로
...
def find_series(key: str) -> EcosSeries:
    ...
    raise ValueError(SERIES_NOT_FOUND_TEMPLATE.format(key=key, available=[s.key for s in ECOS_SERIES]))
