...
# 자금을 3등분해 **동시에** 진입하고 손절선만 다르게 둔다. 손절선은 전부 진입가 기준이며
# 보유 기간 내내 바뀌지 않는다 — 매일 갱신하면 손실이 이어질 때 최악이 무제한으로 열린다.
#
# 단일 손절선은 -3% 와 -4% 사이에서 결과가 29%p 뒤집혔다. 1%p 차이에 그만큼 흔들린다는 것은
# "그 값이 좋다"가 아니라 **표본이 그 지점을 특정할 만큼 크지 않다**는 뜻이므로 분할로 회피한다
# (결정 ⑤). 비율(0.04 = 4%)로 정의한다
STOP_LOSS_LEVELS: Final = (0.04, 0.05, 0.06)

# ============================================================
# 보유 한도
# ============================================================

# 이익이 나면 그날 즉시 전량 청산하고, 손실일 때만 한도까지 끈다.
# **하나를 고르지 않고 세 값을 나란히 산출한다** (결정 ⑥) — 한 포지션은 한도 하나만 가질 수
# 있으므로 이것은 자금을 나누는 축이 아니라 **비교 축**이다
HOLD_LIMITS: Final = (1, 2, 3)
