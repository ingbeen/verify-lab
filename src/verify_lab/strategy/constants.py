...
**매매법별 파라미터는 `strategy/<slug>_constants.py` 가 갖는다.** 전에는 셋이 여기 함께
있었고 그래서 두 가지가 새어 나갔다.

- **공유 체결식의 기본값이 한 매매법의 값**이었다 — `trade_fill.simulate_signal` 의
  `stop_level` 이 역방향의 −5% 라, 인자를 빠뜨리면 다른 매매법이 조용히 그 손절선으로 체결됐다
- **이 모듈이 `studies.reverse.constants` 를 import** 했다. 계약이 금지한 「매매법끼리
  import」를 파일 이름으로는 피했지만, **월말 매매를 돌려도 역방향의 `DATASETS` 가 딸려 왔다**

`tests/test_layer_contracts.py` 가 둘 다 되살아나지 못하게 막는다.
"""
