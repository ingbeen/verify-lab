...
def resolve_positions(trading_days: pd.DatetimeIndex, dates: pd.DatetimeIndex, *, label: str) -> np.ndarray:
    ...
    **`RuntimeError` 인 이유**: 매매 계층은 진입일·청산일을 `trading_days` 자신에서 만든다
    (중간선거_사이클은 `cycle_calendar` 가 그 시세의 거래일로 만든 일정을 넘긴다).
    따라서 `-1` 은 잘못된 입력이 아니라 **일정 모듈의 버그**로만
    생긴다. 같은 사고를 날짜 목록을 **파라미터로 받는** 달력 일정 모듈
    (중간선거_사이클의 `cycle_calendar.month_offset_exit_schedule`)은
