...
# 승률이 손익분기 승률보다 몇 %p 위인가. **두 값이 표에 나란히 있어도 사람이 매번 뺐고**,
# 그러면 행마다 다시 계산해야 해서 그 축으로 정렬할 수도 없다.
#
# **이름을 「여유」·「초과」로 하지 않는다** — 손익분기에 못 미치는 칸이 실재해 음수가 나오는데
# 「여유가 -0.79」는 읽히지 않는다. `.claude/rules/docs.md` 가 같은 이유로 「초과분」을
# 「기준선 대비 차이」로 바꿨다.
#
# **단위가 %p 다** — 두 백분율의 차이이지 백분율이 아니다
DISPLAY_BREAKEVEN_MARGIN: Final = "손익분기 대비(%p)"

DISPLAY_LOSING_COUNT: Final = "질 때 표본"

# 손익비의 **분자와 분모**. 손익비만으로는 그 비율이 어떤 크기의 이익과 손실에서
# 나왔는지 보이지 않는다.
#
# **`이길 때` 는 양수, `질 때` 는 음수다.** `최악(%)` 이 음수인 것과 같은 관용이다.
# 둘 다 절대값으로 내면 표를 읽는 사람이 어느 쪽이 손실인지 이름으로만 판단해야 한다.
#
...
# **잴 수 없는 «이유»**(지수는 종가만 있다 — `docs/조사/월말_진입/설계.md` §7.6)는 값이 아니라
