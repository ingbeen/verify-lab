...
**원칙이 요구하는 것이므로 매매법마다 구현하지 않는다.** 이 산식을 한 매매법의 runner 에
두면 다른 매매법이 그 모듈에서 가져다 쓰게 되고, 그 매매법 사정으로 고칠 때
남의 성적이 조용히 함께 바뀐다.
...
def _period_row(
    ...
    **구간 기간을 함께 낸다.** 구간 이름만으로는 어느 기간인지 알 수 없다 —
    ...
    row: dict[str, Any] = {
        ...
        # **손익비의 분자와 분모를 그대로 낸다.** **`measure` 가 이미 구한 값이라 다시 세지 않는다.**
        #
...
def to_summary_frame(rows: Sequence[Mapping[str, Any]]) -> pd.DataFrame:
    ...
    **건수 컬럼을 결측을 견디는 정수형으로 바꾼다.** 구간 다섯 중 **표본이 0건인 것이
    하나만 생겨도** 그 행의 지표가 결측이라 pandas 가 열 전체를 실수로 만들고, 그러면
    `질 때 표본`·`갭손절`·`장중손절`·`사건` 이 `0.0` 으로 저장된다 — 같은 행의 `신호` 는
    정수인데 말이다. 표본 0건인 구간은 원칙 17 이 **행을 남기라고** 요구하므로 늘 생길 수 있다.

    **빈칸은 빈칸으로 남는다.** `0` 으로 채우면 뒤 절반·최근 N년이 「한 번도 안 걸렸다」고
    거짓으로 주장하게 되는데, 실제로는 「잰 적이 없다」다.
