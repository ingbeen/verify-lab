...
# 검증 산출물. **매매법당 한 폴더**이고 재실행이 그 자리를 덮는다. git 으로 동기화하는 것은
...
# **파일명 규칙은 수집기와 읽는 쪽이 공유하는 계약이다.** 수집·측정·검증·스크립트가 모두
# 같은 이름을 봐야 하며, 한 곳만 달라져도 **예외는 「파일 없음」으로만 뜬다** —
...
# 소급 산출 구간에 시가·고가·저가가 아예 0 이다 (`docs/조사/월말_진입/설계.md` §7.4)
