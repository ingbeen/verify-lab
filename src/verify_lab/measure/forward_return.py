...
**익일 시가 기준은 1일 구간만 낸다** (`docs/spec/역방향_설계.md` §7 결정 ⑳).
...
# 측정 구간 (거래일). docs/spec/역방향_설계.md §4 가 확정한 값이며
# 호출자가 성과를 보며 돌리는 노브가 아니라 상수다.
#
# 단기 구간인 이유는 성과가 아니라 **재는 대상의 성질**이다 (스펙 §7 결정 ㉑).
# 역방향 비율이 답하려는 "반대로 갔는가"는 인버스 상품으로 집행하는데, 그 상품은 일간
# 리밸런싱이라 구간이 길어질수록 원지수 방향과 손익 부호가 어긋난다. 바깥 경계로 1개월을
# 남기는 것은 단기 반전이 이어지는지 되돌아가는지를 볼 기준이 필요하기 때문이다
