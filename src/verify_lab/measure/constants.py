...
#
# **공통 계층에 두는 이유**: 원칙 17 이 모든 검증에 요구하는 것이라 검증마다 새로 만들면
# 같은 원칙이 다른 답을 낸다. 실제로 세 곳(`leverage_tracking`·`strategy`·`futures_leverage`)에
# 같은 문자열이 따로 정의돼 있었다
# **표본 하한은 여기 두지 않는다.** 무엇을 하한으로 삼을지는 축마다 다르고
# (검정 하한·칸당 하한·구간별 하한), 새 상수를 만들면 지금 있는 세 개에 네 번째가 더해진다.
# 각 계층이 자기 하한을 그대로 쓰고 이 컬럼에는 그 판정 결과만 담는다
