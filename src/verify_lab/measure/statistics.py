...
def mean_rate_conflict(frame: pd.DataFrame) -> pd.Series:
    ...
    **공통 계층이 소유한다 — 검증 runner 가 각자 갖지 않는다.** 원칙이 모든 검증에 요구하는
    것을 검증마다 구현하면 같은 원칙이 다른 답을 내고, **docstring 까지 같은 함수**가
    여러 벌 생겨도 한 벌만 고쳐지는 것을 아무것도 막지 못한다.
...
class PayoffProfile(NamedTuple):
    ...
    **분자와 분모도 함께 낸다.** 성적표가 손익비 옆에 그 두 값을 싣는데, 여기서 이미 구한
    값이라 내보내기만 하면 되고, 부르는 쪽이 다시 계산하면 **산식이 두 벌**이 된다.
...
def payoff_profile(
    ...
    **분자가 없는 것과 분모가 없는 것은 뜻이 다르다.**
    ...
    | 진 거래 | 손익비 `NaN` · 손익분기 `NaN` | 수학적으로 무한대라 숫자로 적을 수 없다. **판정에는 쓰이지 않으므로** `NaN` 이 칸을 떨어뜨리지 않는다 — 값은 매매 성적표가 실어 사용자가 직접 읽는다 |
