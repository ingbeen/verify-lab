...
def _dataset_record(context: _Context) -> dict[str, Any]:
    ...
    **앞의 다섯 키는 공통 함수가 만든다 — 같은 사전을 여기서 손으로 조립하지 않는다.**
    조립하면 구현이 검증마다 한 벌씩 생기고, 한 벌만 고쳐도 예외가 나지 않는다.
    뒤의 둘은 이 검증의 축이다.
...
def _measure_reverse_all(
    ...
    **이벤트 정의가 아니라 집계 단계의 합성이다.** 어느 날이 신호인지는 방향별 경로가 이미
    ...
    if not normalized_returns:
        empty_groups.append(_empty_group_record(identity))
        return _ReverseAllBlocks(statistics=[], excess=[], test=[])
...
def _empty_group_record(identity: Mapping[str, Any]) -> dict[str, Any]:
    ...
    **키가 `label` 인 것은 값이 표시 이름이기 때문이다.** 신호군을 가르는 것은 산출물 CSV 의
    `종목` 컬럼이고 그 컬럼에는 표시 이름이 들어간다 — 종목코드가 아니다. 이 키를
    `ticker` 라고 부르면 같은 파일 안에서 한 이름이 두 가지를 가리키게 된다.
