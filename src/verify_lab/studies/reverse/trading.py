...
# **최상위 키는 이 모듈이 갖지 않는다** (`execution/run_summary.py`). 여기서 매매법 이름을
# 값으로 적으면 **폴더 이름과 요약의 이름이 갈린다** — slug 의 정의처는
# `studies/<slug>/constants.py` 의 `TRACK_NAME` 하나다
KEY_TARGETS = "targets"
KEY_LABEL = "label"
...
class StrategyOutputs:
    ...
    **세 매매법이 같은 이름을 쓴다** — 성적표는 `performance`, 실행 요약은 `meta` 처럼
    ...
    Attributes:
        ...
        summary: 실행 요약 (`execution/run_summary.py` 의 틀)
    """
...
def run_reverse_trading(
    ...
    **`stop_levels` 는 체결 경로를 합성 시세로 검사하는 입구다** — 실행 스크립트는 넘기지 않는다.
    ...
    for target in targets:
        ...
        # **제외 건수는 손절선마다 다를 수 있다.** 데이터 끝에 걸린 신호는 무손절이면
        # 한도일을 못 채워 제외되지만, 손절선이 있으면 **D+1 에 갭으로 나가 체결이 만들어진다.**
        # 한 값만 적으면 나머지 행의 표본 보존이 거짓이 된다
        # **키를 문자열로 박는다.** `stop_level_value` 는 음수 실수와 문자열 둘을 내는데,
        # JSON 은 키를 문자열로만 담으므로 그대로 넣으면 **쓸 때와 읽을 때의 타입이 갈린다**
        ...
        # **요약은 격자를 다 돈 뒤에 쌓는다.** 위에서 체결이 없는 손절선을 건너뛰므로,
        # 안에서 쌓으면 전부 제외된 대상의 제외 건수가 어디에도 남지 않는다 (표본 보존)
        target_records.append(_target_record(target, signals, excluded_by_stop))
...
def _identity(target: Target, *, direction: str, stop_level: float | None) -> dict[str, Any]:
    """행을 식별하는 앞 컬럼들을 만든다.
...
def _summary_rows(target: Target, block: _Block, *, stop_level: float | None) -> list[dict[str, Any]]:
    """한 대상의 성적을 구간마다 한 줄씩 만든다.

    **산식은 `execution/periods.py` 가 소유한다.** 구간 5행은 측정의 원칙 17 이 **모든**
...
def _target_record(target: Target, signals: _Signals, excluded_by_stop: dict[str, int]) -> dict[str, Any]:
    ...
    **손절선마다 따로 담는다.** 데이터 끝에 걸린 신호는 무손절이면 한도일을 못 채워
    제외되지만 손절선이 있으면 **갭으로 나가 체결이 만들어진다** — 한 값만 적으면
    나머지 행의 표본 보존이 거짓이 된다.

    Args:
        target: 매매 대상
        signals: 신호 목록
        excluded_by_stop: 손절선 표기 → 그 손절선에서 체결을 만들지 못한 신호 수
    ...
    return {
        ...
        KEY_EXCLUDED_COUNT: excluded_by_stop,
    }
