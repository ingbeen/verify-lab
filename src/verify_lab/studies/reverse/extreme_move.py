...
from verify_lab.studies.reverse.constants import DEFAULT_RANK_CUT, Direction
...
def find_extreme_move_events(
    ...
    **순위를 미리 계산해 넘길 수 있다.** `expanding_rank` 는 방향과 파라미터에 무관하게
    같은 값이므로, 같은 시세로 여러 번 부르는 호출자는 한 번 만들어 돌려 쓴다.
    넘기지 않으면 여기서 계산하므로 **단독 호출은 그대로 동작한다.**
