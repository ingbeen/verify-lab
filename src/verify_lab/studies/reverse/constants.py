...
# **두 종목 모두 K=10 이다** (규칙 문서 결정 ②). K=20 은 대조로도 내지 않는다(결정 ⑭) —
# 지운 K=20 과 손절선 격자의 성적은 규칙 문서 §3.5 가 수치로 갖고, 코드는 git `9126c54` 에 있다.
#
# **종목마다 컷을 가르지 않는다.** 같은 K 라도 데이터 시작일이 달라 순위 축적량이 갈리고
# 등락률 하한이 QQQ 쪽이 높아 QQQ 신호가 드물지만, 컷이 종목마다 다르면 실행에서 실수하기 쉽다
# (결정 ⑬ 과 같은 이유)
#
# **시작연도는 전부 기본값이다** — 구간 축은 닫혀 있다 (결정 ③)
TARGETS: Final = (
