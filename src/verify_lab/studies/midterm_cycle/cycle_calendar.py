"""사이클 달력 — 그 달 «첫» 거래일 진입, N개월 뒤 달의 «마지막» 거래일 청산

[중요] **두 규칙의 휴장 처리 방향이 반대다.**

| | 목표일이 휴장이면 | 왜 |
| --- | --- | --- |
| 진입 (10월 첫 거래일) | **미룬다** | 앞당기면 9월로 넘어가 4분기 밖에서 사게 된다 |
| 청산 (다음해 6월 마지막 거래일) | **앞당긴다** | 미루면 7월로 넘어가 2분기 밖에서 팔게 된다 |

**`measure/calendar_*` 의 기존 함수를 쓸 수 없다.**

- `month_entry_dates` 는 「목표일 «이하»인 거래일 중 가장 늦은 것」이라 **앞당김**이다.
  `calendar_day=1` 로 부르면 10월 1일이 휴장인 해가 전부 「진입일 없음」이 된다 —
  주말만 따져도 7년에 2년꼴이다
- `month_exit_schedule` 은 **진입 달의** 마지막 거래일을 기준점으로 삼는다.
  이 매매법의 청산은 8개월 뒤 달이라 그 기준점으로는 겨눌 수 없다

**그래서 이 매매법이 자기 달력을 갖는다.** 공통 계층으로 올리지 않는 것은
`src/verify_lab/CLAUDE.md` 가 「공통 계층은 확정 후 · 세 번째가 올 때 정한다」로 정했고
**이것이 첫 번째**이기 때문이다.

**신호와 기준선이 같은 청산 규칙을 쓴다.** 진입만 갈린다 — 신호는 10월 첫 거래일,
기준선은 **모든 달의** 첫 거래일이다. 청산을 달리하면 보유 길이가 어긋나 그 차이가
...
| 뺀 달 | 왜 |
| --- | --- |
| 데이터의 **첫** 달 (진입) | 달 중간부터 시작했으면 그 달의 「첫 거래일」이 진짜 첫 거래일이 아니다 |
| 데이터의 **마지막** 달 (청산) | 그 달이 끝났는지 데이터만으로 알 수 없다 — 「데이터가 끊긴 날」을 월말로 삼으면 보유가 짧은 가짜 표본이 된다 |
...
def month_first_entries(trading_days: pd.DatetimeIndex, *, month: int | None = None) -> pd.DataFrame:
    """달마다 **첫 거래일**을 진입일로 삼는다.

    목표일(그 달 1일)이 휴장이면 **미룬다** — 그 달의 첫 거래일이 곧 진입일이다.
    앞당기면 전월로 넘어가 다른 매매가 되므로 방향이 반대인 `month_entry_dates` 를 쓸 수 없다.

    **데이터의 첫 달은 뺀다.** 달 중간부터 시작했으면 그 달의 「첫 거래일」이 진짜 첫 거래일이
    아니다 — 조건부로 넣지 않는 것은 `month_entry_dates` 가 마지막 달을 통째로 빼는 것과
    같은 이유다.
    ...
    # **데이터의 첫 달을 뺀다.** 그 달의 첫 거래일을 데이터만으로는 알 수 없다
    first_month = days[COL_MONTH].iloc[0]
    days = days[days[COL_MONTH] > first_month]

    if days.empty:
        logger.debug(f"데이터가 한 달치뿐이라 진입일이 없습니다 (달 {month})")
        return empty_entries()

    # 그 달의 거래일 중 가장 이른 것이 진입일이다. 목표일이 거래일이면 그날 자신이고
    # 휴장이면 그 이후 첫 거래일이 된다 — 같은 달 안에서만 고르므로 전월로 넘어가지 않는다
    entry_by_month = days.groupby(COL_MONTH, sort=True)[COL_DATE].min()
...
def month_offset_exit_schedule(
    ...
    **데이터의 마지막 달은 청산 달로 쓰지 않는다.** 그 달이 끝났는지 데이터만으로 알 수 없어,
    ...
    Args:
        ...
        entries: `month_first_entries` 가 낸 진입일 표
    ...
    # 3. 진입일 위치. 진입일은 `month_first_entries` 가 거래일에서 골랐으므로 반드시 있다
