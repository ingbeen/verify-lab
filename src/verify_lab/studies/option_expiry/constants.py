...
# 이것을 내지 않는다** — `scripts/run_option_expiry.py --stop-grid` 로 켠다.
...
# 무손절 자체는 아래 `EXPIRY_STOP_GRID` 가 `None` 행으로 따로 낸다
EXPIRY_STOP_LEVELS: Final = tuple(round(0.010 + 0.005 * step, 4) for step in range(19))

# 실행이 도는 두 목록이다. **CLI 도 runner 도 이것을 조립하지 않고 이름으로 고른다** —
# 두 곳에서 만들면 갈리고, 갈려도 예외가 나지 않는다(`meta.json` 이 돌지 않은 격자를 적는다).
EXPIRY_STOP_DEFAULT: Final[tuple[float | None, ...]] = (EXPIRY_STOP_LEVEL,)
# 재선정할 때 도는 격자 — 위 격자에 **무손절 대조축**(`None`)을 더한 것
EXPIRY_STOP_GRID: Final[tuple[float | None, ...]] = (None, *EXPIRY_STOP_LEVELS)
...
#   - **재수집하면 이 목록을 다시 판단한다** (아래 문단)
#
# **손절선 축은 `EXPIRY_STOP_DEFAULT` 가 따로 좁혔고 조건이 셋 더 붙는다** —
# `.claude/rules/trading.md` 「손절선 축도 같은 조건으로 좁힐 수 있습니다」가 SoT다.
#
