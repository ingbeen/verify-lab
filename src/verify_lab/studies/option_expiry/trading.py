"""옵션 만기일 체결 — 대상 칸을 순회해 성적표와 원자료를 조립한다

이 모듈은 **매매 규칙을 계산하지 않는다.** 만기일과 청산일은 같은 패키지의 이벤트 정의가,
...
def run_option_expiry_trading(
    ...
    **확정 손절선 하나만 보려면 `stop_levels=(EXPIRY_STOP_LEVEL,)` 를 넘긴다** —
    ...
    # **성적표 이름은 하나다.** 손절선이 몇 종이든 행이 늘어날 뿐이라 파일을 가르면
    # 같은 뜻의 표가 두 이름으로 불린다 — 월말이 이미 격자를 `성적표.csv` 한 장에 낸다
...
# **`period_rows` 를 재노출하지 않는다.** 이 모듈은 `execution/periods.py` 에서 가져다 쓰는
