...
# yfinance 조회로 확정했다. 근거는 `docs/조사/레버리지_ETF_괴리/설계.md` 에 있다.
#
...
# 3~5% 뿐이라 칸이 성립하지도 않는다. 근거는 `docs/조사/레버리지_ETF_괴리/설계.md`
...
# 분위 컬럼과 그 표시 이름. **재는 쪽과 읽는 쪽이 같은 목록을 본다** — 읽는 쪽에
# `P05`·`P95` 를 손으로 적으면 재는 쪽의 분위를 바꿨을 때 그 열이 조용히 빈다.
#
