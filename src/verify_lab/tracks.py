...
TRACKS: Final = (
    ...
    # 위 둘을 2×2 로 교차해 나온 매매법. 확정 칸의 승률이 사용자 기준에 못 미쳐 걸지 않기로
    # 했고(2026-09-26) 실행 코드를 지웠다 — 위 둘과 같은 자리다. 규칙과 그 근거는
    # `docs/조사/만기_말일/규칙.md` 가 갖고, 되살리는 절차는 같은 폴더의 `결과.md` 에 있다
    Track("expiry_monthend", "만기_말일", GRADE_SURVEY, KIND_PROPERTY),
    # 중간선거 해 4분기 ~ 다음해 2분기를 통으로 드는 매매법. **걸기로 정했으므로 매매 등급**이며
    # 확정 규칙은 `docs/매매/중간선거_사이클/규칙.md` §1 이다. 보유가 9개월이라 다른 매매법(며칠 ~ 2주)과
