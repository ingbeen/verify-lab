...
# 기준선은 **판정에 쓰이지 않지만 판정 결과를 읽는 데 필요하다.** 같은 적중률 60% 가
# 오르는 쪽에서는 기준선(55~59%) 대비 +1~5%p 에 불과하고 내리는 쪽에서는 +15~19%p 다.
# 기준선과 사실상 같은 칸도 게이트를 통과하므로(SPY 11월 — 적중률 66.7% · 기준선 66.7%)
# 이 두 열이 없으면 그 칸을 다른 후보와 구별할 수 없다
