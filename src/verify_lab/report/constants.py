...
#
# [중요] **`통계.csv` 와 공존한다.** 옵션 만기일이 확정 칸으로 좁아지며 측정 표 여덟을
# 한 장으로 합친 것이고, 역방향·월말은 아직 `통계.csv` 를 낸다 — 그쪽은 격자 전체를
# 재는 중이라 축별 집계표가 그대로 필요하다. **위 「한글 넷이 같은 이름」과 어긋나는
# 과도기이며**, 나머지 둘을 같은 구조로 옮길 때 해소된다
