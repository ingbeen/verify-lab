...
- **본인 변경으로 생긴 orphan만 정리한다**: 사전에 존재하던 데드 코드는 사용자 요청 없이 삭제하지 않는다.
...
- **약한 성공 기준("동작하게 만들기")은 매 단계 사용자 확인이 필요하다**.
...
- `docs/plans/`를 **비울 때는 `.gitkeep`을 되살립니다** — 디렉토리가 사라지면 계획서 게이트가 꺼집니다.
  계획서가 하나라도 있는 동안에는 `.gitkeep`이 없어도 됩니다 ("디렉토리 구조"의 ".gitkeep 의 수명" 참고)
...
`.gitkeep`은 **폴더가 비어 있는 동안 그 폴더를 git에 남겨두기 위한 자리표시자**입니다.
git은 파일만 추적하므로 빈 폴더는 clone 하면 사라집니다. 역할이 그것뿐이므로 아래처럼 다룹니다.

- 해당 폴더에 `.gitkeep` 외의 파일이 생기면 **`.gitkeep`을 삭제해도 됩니다.** 그 파일이 폴더를 유지합니다
- 반대로 폴더를 다시 비울 때는 **`.gitkeep`을 되살립니다**

`docs/plans/`는 계획서를 주기적으로 전부 비우므로 이 되살리기가 특히 중요합니다.
디렉토리가 없으면 계획서 게이트 훅이 스스로 꺼지며(`plan_lint.py`가 `docs/plans/` 존재로 규약 채택을 판정),
꺼졌다는 사실이 아무 데도 표시되지 않습니다.
...
> CRITICAL: 스크립트는 **성격에 따라 실행 주체가 다릅니다.**

| 대상 | 실행 주체 | 이유 |
| --- | --- | --- |
| `scripts/studies/` (검증 실행) | **AI 모델이 직접 실행 가능** | 파라미터를 바꿔가며 수십 번 돌리는 것이 검증의 본질 |
| `scripts/data/` (데이터 수집) | **사용자만 실행** | yfinance·KRX 외부 서버에 실제 요청. 레이트리밋·차단 위험 |
| `validate_project.py` | AI 모델이 직접 실행 가능 | 품질 게이트 |

- AI 모델은 수집 스크립트를 실행하지 않습니다. 코드를 작성하고 실행 방법을 안내합니다.
- 수집 결과가 없어 검증을 진행할 수 없으면, 필요한 명령어를 안내하고 사용자 실행을 기다립니다.
