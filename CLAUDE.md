...
우위 후보로 올리려면 **네 기준을 모두** 통과해야 합니다.

| # | 기준 | 왜 필요한가 |
| --- | --- | --- |
| 1 | **기준선 대비 차이가 ±10%p 이상** (방향 무관, 절대값) | 같은 "60%"가 방향에 따라 뜻이 정반대입니다. 오르는 쪽 기준선은 55~59%, 내리는 쪽은 41~45%. 이 기준이 없으면 오르는 쪽이 부당하게 쉽게 통과합니다 |
| 2 | **방향 적중률 60% 이상** | 크기가 커도 적중률이 낮으면 집행할 수 없습니다 |
| 3 | **우연확률 0.05 미만** (양측 순열 검정) | |
| 4 | **시기를 쪼개도 모든 구간이 적중률 55% 이상** | 한 시기가 만든 값을 걸러냅니다. 구간 수는 **칸당 유효 표본 10건 이상을 유지하는 최대치**로 정합니다 — 3구간이 안 되면 2구간, 2구간도 안 되면 쪼개지 않고 그 사실을 적습니다(원칙 12 와 같은 하한) |

**네 기준을 통과해도 다중 비교와 표본 독립성은 따로 봅니다**(원칙 3·5·7).
**통과는 후보 자격이지 발견이 아닙니다.**
...
실물 사례: 원칙 11(방향 무관 판정)의 후보 판정 기준 네 개가 규격만 있고 구현이 없던 시기가 있었고,
