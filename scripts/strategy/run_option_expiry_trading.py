...
`docs/research/옵션_만기일.md` 0장에서 **게이트를 넘은 칸**에 손절선 격자를 걸어 성적을 낸다.
맨몸 성적은 그 문서가 이미 냈고, 여기가 더하는 것은 **손절 하나**다.
대상 목록의 SoT 는 `strategy/constants.py` 의 `EXPIRY_CELLS` 다.

**통계량이 얇은 칸도 뺀 것이 없다.** 게이트를 넘었으면 함께 내며, 통계량으로 빼면
60칸에서 좋아 보이는 칸만 고르는 사후 선택이 된다 (`docs/spec/옵션_만기일_설계.md` 결정 ㊳).

**손절선은 확정값 -5% 하나다** (`EXPIRY_STOP_LEVEL`). 값을 인자로 열지 않는다 —
값을 옮겨 가며 성적을 보면 표본에 맞춘 튜닝이 된다. 고른 근거와 탈락안은
`docs/spec/옵션_만기일_설계.md` 결정 ㊴ 에 있다.
