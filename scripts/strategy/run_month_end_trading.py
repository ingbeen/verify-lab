...
**기본 대상은 `DATASETS_TRADING` 이 정한다** — 시장 범위는 검증 계층(`DATASETS`)과 같고
인버스만 뺀다. 인버스는 확정 전 교차검증용이라 `--ticker` 로 지목해야 돈다.
**시장 범위는 검증보다 좁히지 않는다** — 코스닥만 돌면 코스피에만 있는 11월 칸의
손절 성적이 통째로 빠진다.
...
# **이 건수는 `docs/research/월말_진입.md` 머리말의 데이터 기간에 묶여 있다** —
# 시세를 재수집하면 달라지므로, 이 셋을 다시 고를 때는 그 문서부터 본다
